...
| `go.yml` | `**/go/**` | Finds every folder containing a `go.mod`, then, for each one, checks `gofmt`, runs `go vet` and `staticcheck`, builds, runs `go test -race` and uploads the binary as a run artifact. The Go version is read from `go.mod`. |
